...
rule pre_train:
    ...
    run:
        train_model(input[0], config['pt_epochs'], config['hidden_size'], loss_func=loss_function,
                    ...
                    learning_rate=0.005, train_type = 'pre', recurrent_dropout=0.3,early_stop_patience=None, seed = config['seed'])
...
rule finetune_train:
    ...
    run:
        train_model(input[0], config['ft_epochs'], config['hidden_size'], loss_func=loss_function,
                    ...
                    learning_rate=0.01, recurrent_dropout=0.3, train_type = 'finetune', early_stop_patience=config['early_stopping'], seed = config['seed'])
