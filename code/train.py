...
csv_log_pre = tf.keras.callbacks.CSVLogger(
    os.path.join(out_dir, f'pretrain_log{tag}.csv'))
...
out_time_file = os.path.join(out_dir, f'training_time{tag}.txt')
with open(out_time_file, 'w') as f:
    f.write(f'elapsed time pretrain (includes building graph):\
             {pre_train_time_elapsed} \n')

model.save_weights(os.path.join(out_dir, f'pretrained_weights{tag}/'))
...
csv_log_ft = tf.keras.callbacks.CSVLogger(
    os.path.join(out_dir, f'finetune_log{tag}.csv'))
...
model.save_weights(os.path.join(out_dir, f'trained_weights{tag}/'))
